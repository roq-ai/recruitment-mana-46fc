...
import { CandidateListPage as ShowCaseList } from 'pages/candidates';
...
function DashboardPage() {
  ...
  const interviewStat = useSWR<PaginatedInterface<InterviewInterface>>(
    () => `/interviews`,
    () => getInterviews({ limit: 1, offset: 0 }),
  );

  const feedbackStat = useSWR<PaginatedInterface<FeedbackInterface>>(
    () => `/feedbacks`,
    () => getFeedbacks({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Interviews',
      result: interviewStat?.data?.totalCount,
      isLoading: interviewStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Feedbacks',
      result: feedbackStat?.data?.totalCount,
      isLoading: feedbackStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`candidates`} passHref legacyBehavior>
                  <Button
