...
type ColumnType = ColumnDef<CandidateInterface, unknown>;
...
export function CandidateListPage(props: CandidateListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getCandidates({
        relations: ['user', 'interview.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CandidateInterface>>(
    () => `/candidates?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CandidateInterface) => {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'resume', header: 'Resume', accessorKey: 'resume' },
    ...
    hasAccess('interview', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.interview,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/candidates/view/${record.id}`} passHref legacyBehavior>
                <Button
