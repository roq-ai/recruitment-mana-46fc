  ...
  const fetcher = useCallback(
    async () =>
      getFeedbacks({
        ...
        order: params.order,
        searchTermKeys: ['feedback_notes.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.feedback_date ? format(parseISO(record?.feedback_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'feedback_notes', header: 'Feedback Notes', accessorKey: 'feedback_notes' },
    ...
    hasAccess('interview', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/interviews/view/${record.interview?.id}`}>
              {record.interview?.interview_time}
            </Link>
          ),
        }
      : null,
