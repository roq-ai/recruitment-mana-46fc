...
type ColumnType = ColumnDef<InterviewInterface, unknown>;
...
export function InterviewListPage(props: InterviewListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getInterviews({
        relations: ['candidate', 'user', 'feedback.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['interview_time.contains', 'interview_type.contains', 'interview_status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<InterviewInterface>>(
    () => `/interviews?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: InterviewInterface) => {
    if (hasAccess('interview', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/interviews/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.interview_date ? format(parseISO(record?.interview_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'interview_time', header: 'Interview Time', accessorKey: 'interview_time' },
    { id: 'interview_type', header: 'Interview Type', accessorKey: 'interview_type' },
    { id: 'interview_status', header: 'Interview Status', accessorKey: 'interview_status' },
    hasAccess('candidate', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/candidates/view/${record.candidate?.id}`}>
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="interview" />
        </Flex>

        {hasAccess('interview', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/interviews/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
